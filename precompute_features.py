...
place of the raw cutouts (spectrum arrays and the manifest are copied through unchanged). Training and
evaluation then use a config with ``image_encoder.type: identity`` pointed at the feature dataset,
which loads features directly - the per-step image load, resize, and CLIP forward disappear.
...
def main() -> None:
    ...
    for shard_name in shard_names:
        ...
        np.savez_compressed(
            ...
            spectrum_wavelength=spectrum_wavelength,
        )
        logger.info("Wrote %s: features %s", shard_name, features_array.shape)
